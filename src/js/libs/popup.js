...
export class Popup {
	...
	eventsPopup() {
		...
		document.addEventListener("click", function (e) {
			...
			if (buttonOpen) {
				e.preventDefault();
				if (buttonOpen.hasAttribute('data-video')) {
					const popupVideo = document.getElementById('popup_video');


					let value = buttonOpen.getAttribute(this.options.buttonYoutube);
					let valueSubstringed = value.substring(value.indexOf('?v=') + 3);
					popupVideo.setAttribute('data-youtube', valueSubstringed);
				}
